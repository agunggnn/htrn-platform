...
export default async function InvoiceDetailPage({ params }: Props) {
  ...
  const paymentList = (payments as unknown as Payment[]) ?? []
  const comp = company as unknown as CompanyProfile
  const bnk = bank as unknown as BankAccount
    ...
    <div className="px-8 py-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span
            className={`rounded-full px-2.5 py-1 text-xs font-medium ${STATUS_STYLE[inv.status] ?? 'bg-gray-100 text-gray-600'}`}
          >
            {inv.status}
          </span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-8">
            ...
            <div className="mb-6 flex items-start justify-between border-b border-gray-100 pb-6">
              <div>
                ...
                <p className="font-bold text-gray-900">{comp?.company_name ?? 'Haturan'}</p>
                {comp?.address && <p className="text-xs text-gray-400">{comp.address}</p>}
              </div>
              <div className="text-right">
                <p className="text-2xl font-bold" style={{ color: '#1a472a' }}>
                  {isID ? 'INVOICE' : 'COMMERCIAL INVOICE'}
                </p>
                <p className="mt-1 text-sm text-gray-500">No: {inv.inv_number}</p>
            ...
            <div className="mb-6 grid grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-sm font-semibold">{comp?.company_name ?? 'Haturan'}</p>
                {comp?.npwp && <p className="text-xs text-gray-500">NPWP: {comp.npwp}</p>}
              </div>
              {buyer && (
                <div>
                  <p className="mb-1 text-xs font-medium tracking-wide text-gray-400 uppercase">
                  ...
                  {buyer.contact_name && (
                    <p className="text-xs text-gray-500">Attn: {buyer.contact_name}</p>
                  )}
            ...
            <table className="mb-4 w-full text-sm">
              <thead>
                <tr className="border-b-2 border-gray-800">
                  <th className="pb-2 text-left text-xs font-semibold text-gray-700">
                    Description
                  </th>
                  <th className="pb-2 text-left text-xs font-semibold text-gray-700">Grade</th>
                  <th className="pb-2 text-left text-xs font-semibold text-gray-700">HS Code</th>
                  <th className="w-24 pb-2 text-right text-xs font-semibold text-gray-700">
                    Qty (kg)
                  </th>
              ...
              <tbody>
                {((lineItems as unknown as InvoiceItem[]) ?? []).map((l) => (
                  <tr key={l.id} className="border-b border-gray-50">
                    <td className="py-2.5">
                      <p className="font-medium">{l.description}</p>
                      <p className="text-xs text-gray-400">
                        Country of Origin: {l.country_of_origin}
                      </p>
                    </td>
            ...
            <div className="mb-6 flex justify-end">
              <div className="w-56 space-y-1 text-sm">
                <div className="flex justify-between text-gray-500">
                  <span>Subtotal</span>
                  <span>{fmt(inv.subtotal ?? 0)}</span>
                </div>
                {inv.tax_rate > 0 && (
                  <div className="flex justify-between text-gray-500">
                    <span>Tax ({inv.tax_rate}%)</span>
                    <span>{fmt(inv.tax_amount ?? 0)}</span>
                  </div>
                )}
                  ...
                  <div className="flex justify-between text-xs text-gray-400">
                    ...
                    <span>
                      ...
                      {new Intl.NumberFormat('id-ID').format(
                        Math.round((inv.total_amount ?? 0) * inv.exchange_rate)
                      )}
                    </span>
                  </div>
                )}
                <div className="flex justify-between border-t border-gray-200 pt-1.5 font-bold text-gray-900">
                  <span>TOTAL</span>
                  <span>{fmt(inv.total_amount ?? 0)}</span>
                </div>
              </div>
            </div>
            ...
            {bnk?.bank_name && (
              <div className="mb-4 border-b border-gray-100 pb-4">
                <p className="mb-1 text-xs font-medium text-gray-400">Payment Instructions</p>
                <p className="text-sm text-gray-700">
                  {bnk.bank_name} · {bnk.account_number} · {bnk.account_name}
                  {inv.currency !== 'IDR' && inv.exchange_rate > 1 && (
                    <span className="ml-2 text-xs text-gray-400">
                      ({inv.currency} 1 = IDR{' '}
                      {new Intl.NumberFormat('id-ID').format(inv.exchange_rate)})
                    </span>
                  )}
                </p>
              </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-5">
            ...
            <p className="mb-1 text-2xl font-bold text-red-600">{fmt(inv.amount_due ?? 0)}</p>
            <p className="mb-3 text-xs text-gray-400">
              outstanding dari {fmt(inv.total_amount ?? 0)}
            </p>
            {/* Progress bar */}
            <div className="mb-1 h-2 w-full rounded-full bg-gray-100">
              <div
                className="h-2 rounded-full"
                style={{ width: `${Math.min(100, paidPct)}%`, backgroundColor: '#1a472a' }}
              />
            </div>
            <p className="text-xs text-gray-400">
              {paidPct.toFixed(0)}% dibayar ({fmt(inv.amount_paid ?? 0)})
            </p>
          </div>
