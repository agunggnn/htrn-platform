...
import type { Buyer, Signatory, Quotation, QuotationItem } from '@/types'
...
type QuotationPrefillItem = QuotationItem & {
  items: { name: string; name_en: string | null; hs_code: string | null } | null
}
export type InvoicePrefill = Quotation & { quotation_items: QuotationPrefillItem[] }
type Props = { buyers: Buyer[]; signatories: Signatory[]; prefill?: InvoicePrefill | null }
...
export function InvoiceForm({ buyers, signatories, prefill }: Props) {
  ...
  const [lines, setLines] = useState<LineItem[]>(() => {
    if (prefill?.quotation_items.length) {
      return prefill.quotation_items.map((li) => ({
        description:
          (language === 'id' ? li.items?.name : (li.items?.name_en ?? li.items?.name)) ?? '',
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4 rounded-2xl border border-gray-200 bg-white p-6">
        <div className="col-span-2">
          <label className="mb-1 block text-xs font-medium text-gray-500">Buyer *</label>
          <select
            value={buyerId}
        ...
        {[
          { label: 'Tanggal Invoice', val: issueDate, setter: setIssueDate, type: 'date' },
          { label: 'Jatuh Tempo', val: dueDate, setter: setDueDate, type: 'date' },
        ...
        {currency !== 'IDR' && (
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-500">Kurs ke IDR</label>
            <input
              type="number"
              value={exchangeRate}
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-gray-500">Bahasa</label>
          <select
            value={language}
        ...
        {signatories.length > 0 && (
          <div>
            <label className="mb-1 block text-xs font-medium text-gray-500">Penandatangan</label>
            <select
              value={signatoryId}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              ...
              <th className="w-20 px-3 py-2.5 text-left text-xs font-semibold text-gray-500">
                Grade
              </th>
              ...
              <tr key={idx} className="border-b border-gray-50">
                <td className="px-4 py-2">
                  <input
                    ...
                    onChange={(e) =>
                      setLines(
                        lines.map((x, i) => (i === idx ? { ...x, description: e.target.value } : x))
                      )
                    }
                ...
                <td className="px-3 py-2">
                  <input
                    ...
                    onChange={(e) =>
                      setLines(
                        lines.map((x, i) => (i === idx ? { ...x, grade_code: e.target.value } : x))
                      )
                    }
                    placeholder="A"
                    className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm focus:ring-1 focus:ring-green-700 focus:outline-none"
                ...
                <td className="px-3 py-2">
                  <input
                    ...
                    onChange={(e) =>
                      setLines(
                        lines.map((x, i) => (i === idx ? { ...x, quantity: e.target.value } : x))
                      )
                    }
                    placeholder="0"
                    className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm focus:ring-1 focus:ring-green-700 focus:outline-none"
                ...
                <td className="px-3 py-2">
                  <input
                    ...
                    onChange={(e) =>
                      setLines(
                        lines.map((x, i) => (i === idx ? { ...x, unit_price: e.target.value } : x))
                      )
                    }
                    placeholder="0.00"
                    className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm focus:ring-1 focus:ring-green-700 focus:outline-none"
                ...
                <td className="px-3 py-2">
                  <input
                    ...
                    onChange={(e) =>
                      setLines(
                        lines.map((x, i) => (i === idx ? { ...x, hs_code: e.target.value } : x))
                      )
                    }
                    placeholder="0907"
                    className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm focus:ring-1 focus:ring-green-700 focus:outline-none"
              ...
              <div className="flex justify-between text-xs text-gray-400">
                ...
                <span>
                  ...
                  {new Intl.NumberFormat('id-ID').format(
                    Math.round(total * parseFloat(exchangeRate))
                  )}
                </span>
              </div>
            )}
